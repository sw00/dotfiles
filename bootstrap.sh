...
stow_dir() {
    ...
    # Dry-run: collect paths of conflicting regular files and remove them.
    local conflicts
    conflicts=$(
        stow -n --no-folding -d "$parent" -t "$HOME" "${pkgs[@]}" 2>&1 \
        | grep -oP '(?<=existing target is neither a link nor a directory: ).*' \
        || true
    )
    if [[ -n "$conflicts" ]]; then
