...
- nvim-treesitter v1.0 changed parser installation API: `ensure_installed` was renamed to `install` on the top-level module (`require('nvim-treesitter').install { ... }` not `require('nvim-treesitter.install').ensure_installed(...)`), and takes a table `{ ... }` argument, not varargs.
