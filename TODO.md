...
- [x] [P2] **model-switch opportunistic switch-back on 2xx.** Done, cooldown-gated (T3): the 2xx `after_provider_response` path switches back from a fallback twin only after an in-process hop, past `rateLimitFallbackRecoveryCooldownSec` (floored by `retry-after`), and not when the twin was user-chosen (`model_select` tracking). Recovery and fallback have independent 2s throttles. Verify on any host: force a 429, let primary recover past the cooldown, confirm the next 2xx returns to primary and `defaultModel` unsticks.
